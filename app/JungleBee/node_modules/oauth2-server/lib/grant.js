...
var debug = require('debug')('JungleBee:grant.js')
 ...
 * @param {Function} next
 */
function Grant (config, req, res, next, websiteLogin) {
    if (websiteLogin) this.websiteLogin = websiteLogin;
    this.config = config;
...
function checkClient (done) {
  ...
  this.model.getClient(this.client.clientId, this.client.clientSecret,
    ...
    self.req.clientId = client.clientId;
    done();
  });
}
...
function usePasswordGrant (done) {
  ...
  if (this.websiteLogin){
      return this.model.getUserObject(uname, pword, function (err, user) {
...
function sendResponse (done) {
  ...
  if (!this.websiteLogin){
      ...
      debug('website Login successful');
